...
async function getArchiveSection(date: string, section: string) {
  try {
    const res = await fetch(`${API_BASE}/api/editions/${date}/${section}`, {
      cache: "no-store",
    });
    if (!res.ok) return null;
    return await res.json();
  } catch (err) {
    console.error(`[Archive Section] Failed to fetch date ${date} section ${section}:`, err);
    return null;
  }
}

export default async function DailyArchiveSectionPage({
  params,
}: {
  params: Promise<{ date: string; section: string }>;
}) {
  const { date, section } = await params;
  const data = await getArchiveSection(date, section);

  if (!data || !data.data.articles || data.data.articles.length === 0) {
    return (
      <div className="container" style={{ padding: "80px 0", textAlign: "center" }}>
        <h2 style={{ fontSize: "28px", marginBottom: "16px" }}>Section Archive Not Found</h2>
        <p style={{ color: "var(--text-secondary)", marginBottom: "32px" }}>
          We couldn't find any articles for section "{section}" on {date}.
        </p>
        <Link href={`/daily/${date}`} className="btn btn-primary">
          Return to {date} Edition
        </Link>
      </div>
    );
  }
    ...
    <div className="container" style={{ padding: "40px 0 80px 0" }}>
      <div style={{ marginBottom: "32px", display: "flex", gap: "16px" }}>
        ...
        >
          ...
          <span>Back to {date} Edition</span>
        </Link>
      </div>
      ...
      <main>
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "14px", color: "var(--text-muted)" }}>
            ...
            <span>Edition Date: {new Date(date).toLocaleDateString()}</span>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "32px" }}>
          {articles.map((item: any) => {
            const senditUrl = item.redirect_hash
              ? `${API_BASE}/sendit/${item.redirect_hash}?src=web_section`
              : item.canonical_url;
            return (
              <article
                key={item.id}
