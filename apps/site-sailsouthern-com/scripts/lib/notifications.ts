...
const SLACK_WEBHOOK    = process.env.SLACK_WEBHOOK || process.env.SLACK_WEBHOOK_ALERTS || process.env.SLACK_WEBHOOK_HQ || process.env.MONITORING_WEBHOOK_URL || "";
...
function sendTelegramAlert(msg: AlertMessage, promises: Array<Promise<any>>) {
  ...
  promises.push(
    fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
      ...
      .then(async (res) => { if (!res.ok) console.warn(`[Notification] Telegram failed: ${res.status}`); })
      .catch((err) => console.error("[Notification] Telegram error:", err))
  );
}
...
function sendDiscordAlert(msg: AlertMessage, promises: Array<Promise<any>>) {
  ...
  promises.push(
    fetch(DISCORD_WEBHOOK, {
      ...
      .then(async (res) => { if (!res.ok) console.warn(`[Notification] Discord failed: ${res.status}`); })
      .catch((err) => console.error("[Notification] Discord error:", err))
  );
}
...
function sendSlackAlert(msg: AlertMessage, promises: Array<Promise<any>>) {
  ...
  promises.push(
    fetch(SLACK_WEBHOOK, {
      ...
      .then(async (res) => { if (!res.ok) console.warn(`[Notification] Slack failed: ${res.status}`); })
      .catch((err) => console.error("[Notification] Slack error:", err))
  );
}
...
function sendNtfyAlert(msg: AlertMessage, promises: Array<Promise<any>>) {
  ...
  promises.push(
    fetch(`${NTFY_URL}/${NTFY_TOPIC}`, {
      ...
      .then(async (res) => { if (!res.ok) console.warn(`[Notification] ntfy failed: ${res.status}`); })
      .catch((err) => console.error("[Notification] ntfy error:", err))
  );
}
...
function sendEmailAlert(msg: AlertMessage, promises: Array<Promise<any>>) {
  ...
  promises.push(
    resend.emails.send({
      ...
      .then(({ error }) => { if (error) console.warn(`[Notification] Email alert failed: ${error.message}`); })
      .catch((err) => console.error("[Notification] Email alert error:", err))
  );
}

// SMS fires only on warn/block — routine info events do not warrant a text message.
function sendSmsAlert(msg: AlertMessage, promises: Array<Promise<any>>) {
  ...
  promises.push(
    fetch(`https://api.twilio.com/2010-04-01/Accounts/${TWILIO_SID}/Messages.json`, {
      ...
      .then(async (res) => { if (!res.ok) console.warn(`[Notification] SMS failed: ${res.status}`); })
...
export async function broadcastAlert(msg: AlertMessage) {
  ...
  if (promises.length > 0) {
    ...
    console.log(`[Notification (Offline)]: ${enriched.title ? `[${enriched.title}] ` : ""}${enriched.text}`);
  }
}
