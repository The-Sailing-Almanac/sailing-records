...
**Batch 1 structural hygiene completed 2026-05-18:**
- Folder structure created: `ingestion/`, `analysis/`, `schema/`, `verify/`, `archive/`, `docs/`, `handoffs/`
- All scripts moved to appropriate directories (using `git mv`; git history preserved)
- README.md trimmed to short orientation layer; detail moved to `docs/`
- `docs/sources.md`, `docs/schema.md`, `docs/pipeline.md` created
- `archive/README.md` created explaining contents and notable reference material
- `requirements.txt` added
- `_scratch/` POD and NAAG files removed from repo
- `diagnostic/` HTML specimens now tracked in git (removed from .gitignore)
- `HANDOFF.md` moved to `handoffs/HANDOFF-2026-05-18.md`

**Still needed before first push:**
- Set remote origin: `git remote add origin <your-remote-url>`
- Confirm `requirements.txt` is complete (run `pip freeze` in clean venv to pin versions)
