...
Reads Regatta Network URLs from sailing_urls.db (platform='RegattaNetwork',
status='ok') and fetches each into raw_rn/{regatta_id}/results.html.
...
def folder_complete(event_dir):
    return (os.path.exists(os.path.join(event_dir, "results.html"))
            or os.path.exists(os.path.join(event_dir, "_missing.flag"))
            or os.path.exists(os.path.join(event_dir, "_error.flag")))
...
def process_url(url):
    ...
    event_dir = os.path.join(RAW_DIR, rid)
    ...
    write_file(os.path.join(event_dir, "results.html"), html)
    return "ok"
...
def main():
    ...
    cur = con.cursor()
    cur.execute("""
        SELECT url FROM registry
        WHERE platform = 'RegattaNetwork' AND status = 'ok'
        ORDER BY url
    """)
    ...
    print(f"Processing {len(urls):,} RegattaNetwork URLs ...", flush=True)
